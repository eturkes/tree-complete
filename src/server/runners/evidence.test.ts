...
describe('validateRunnerEvidence', () => {
  ...
  it('normalizes legacy preview estimates to explicit simulation', () => {
    expect(validateRunnerEvidence({ ...validPreviewEvidence, changeKind: 'estimated' }, 'preview'))
  ...
  it.each([
    ...
    [{ ...validPreviewEvidence, checks: [{ ...validPreviewEvidence.checks[0], status: 'passed' }] }, /does not match preview/],
